...
Token *tokenize(const char *expression) {
    ...
    while (*c) {
        ...
        if (*c == '(')
            ...
            //token.value = strndup(c, 1);//MIRAR ACA
            char *new = malloc (1);
            if (new == NULL)
              return NULL;
            new[1] = '\0';
            memcpy (new, c, 1);
            token.value = (char*)new;
...
void show_error(Status status) {
    ...
    switch (status) {
        ...
        case ERROR_OPEN_PARENTHESIS:
            ...
            message = "Faltan paréntesis";
            break;
        case ERROR_CLOSE_PARENTHESIS:
            //message = "Extra parenthesis";
            message = "Demasiados paréntesis";
            break;
        ...
        case ERROR_FUNCTION_ARGUMENTS:
            ...
            message = "Función con argumentos faltantes";
            break;
        case ERROR_UNDEFINED_CONSTANT:
            //message = "Unknown constant";
            message = "Constante desconocida";
            break;
        default:
